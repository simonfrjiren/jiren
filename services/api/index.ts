...
class FetchInstance {
  ...
  handlerError = async ({ err, reject, ...arg }: any) => {
    console.log("handler err", arg, err.response._data);
    reject(err);
  };
  ...
  get = <ResponseType>(url: string, config?: FetchOptions) => {
    return new Promise(
      async (
        res: (value: { data: ResponseType }) => void,
        reject: (axiosError: AxiosError) => void
      ) => {
        ofetch(this.getUrl(url), {
          method: "GET",
  ...
  getOtherUrl = <ResponseType>(url: string) => {
    return new Promise(
      async (
        res: (value: FetchResponse<ResponseType>) => void,
        reject: (axiosError: AxiosError) => void
      ) => {
        axios
